# Nadine — Personal Portfolio

A responsive personal portfolio website built with **HTML, CSS, and JavaScript** and deployed with **GitHub Pages**.

🔗 **Live site:** https://nadine-codes.github.io

## Pages
- **Home** (`index.html`) — introduction and bio
- **About** (`about.html`) — background, skills, education, and experience
- **Projects** (`projects.html`) — selected work with links to source and demos
- **Contact** (`contact.html`) — contact details and a message form

## Features
- Responsive, mobile-first design
- Sticky navigation bar with a mobile menu toggle
- Semantic, accessible HTML
- Custom CSS with a consistent design system
- Interactive contact form (client-side)

## Project Structure
```
.
├── index.html
├── about.html
├── projects.html
├── contact.html
├── style.css
├── script.js
└── images/
    └── profile.jpeg
```

## Running Locally
Open `index.html` in any web browser, or serve the folder with a simple HTTP server:
```bash
python3 -m http.server 8000
```
Then visit `http://localhost:8000`.
